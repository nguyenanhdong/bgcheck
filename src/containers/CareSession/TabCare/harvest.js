import React, { Component, useEffect, useState, useCallback,forwardRef,useImperativeHandle } from "react";
...
const App = forwardRef((props,ref) => {
    ...
    const addRow = () => {
        props.addRow('AddHarvest',{DotChamSoc_Id:props.id,refreshTab:props.refreshTab});
    }
    useEffect(() => {
        getdata();
    }, []);
    ...
    const getdata = ()=>{
        ...
        if(loading) return;
        setLoading(true);
        console.log('url GetThuHoachById',`${BASE_URL}API/GetThuHoachById?id=${props.id}&dep=${userInfo?.DepartmentId}`)
        axios.get(`${BASE_URL}API/GetThuHoachById?id=${props.id}&dep=${userInfo?.DepartmentId}`, {}, {
            headers: {
    ...
    const actionDelete = item => {
        if(loading) return;
        setLoading(true);
        axios.post(`${BASE_URL}API/DeleteThuHoach`, {Id:item.Id}, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                'X-localization': 'vi'
    ...
    const deleteItem = item => {
        Alert.alert(
            `Bạn có thật sự muốn xoá đợt thu hoạch này`,
            '',
            [
                {
                    text: 'Huỷ',
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <NormalFlashList
                    ...
                    emptyViewProps={{
                        bigTitle: 'Danh sách thu hoạch rỗng',
                        bigTitleStyle: {
                            color: 'black',
                            fontSize:14
                        }
