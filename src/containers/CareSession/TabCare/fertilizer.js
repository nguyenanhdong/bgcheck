...
const App = forwardRef((props,ref) => {
    ...
    const getdata = ()=>{
        if(loading) return;
        setLoading(true);
        axios.get(`${BASE_URL}API/GetPhanBonById?id=${props.id}&dep=${userInfo?.DepartmentId}`, {}, {
            headers: {
    ...
    const addRow = () => {
        props.addRow('AddFertilizer',{DotChamSoc_Id:props.id,refreshTab:props.refreshTab});
    }

    const actionDelete = item => {
        if(loading) return;
        setLoading(true);
        axios.post(`${BASE_URL}API/DeletePhanBon`, {Id:item.Id}, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                'X-localization': 'vi'
            ...
            .then((response) => {
                ...
                if (response.status == 200) {
                    showMessage({
                        message: 'Xoá đợt phân bón thành công',
                        duration: 3000,
                        type: "success",
                        icon: 'success'
                      });
    ...
    const deleteItem = item => {
        Alert.alert(
            `Bạn có thật sự muốn xoá đợt phân bón này`,
            '',
            [
                {
                    text: 'Huỷ',
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <NormalFlashList
                ...
                emptyViewProps={{
                    bigTitle: 'Danh sách phân bón rỗng',
                    bigTitleStyle: {
                        color: 'black',
                        fontSize: 14
                    }
