...
const App = forwardRef((props,ref) => {
    ...
    const getdata = ()=>{
        if(loading) return;
        setLoading(true);
        axios.get(`${BASE_URL}API/GetBVTVById?id=${props.id}&dep=${userInfo?.DepartmentId}`, {}, {
            headers: {
            ...
            .then((response) => {
                console.log('response GetBVTVById', response);
    ...
    const addRow = () => {
        props.addRow('AddprotectProduct',{DotChamSoc_Id:props.id,refreshTab:props.refreshTab});
    }
    const actionDelete = item => {
        if(loading) return;
        setLoading(true);
        axios.post(`${BASE_URL}API/DeleteBVTV`, {Id:item.Id}, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                'X-localization': 'vi'
            ...
            .then((response) => {
                ...
                if (response.status == 200) {
                    showMessage({
                        message: 'Xoá đợt sử dụng thuốc BVTV thành công',
                        duration: 3000,
                        type: "success",
                        icon: 'success'
                      });
    ...
    const deleteItem = item => {
        Alert.alert(
            `Bạn có thật sự muốn xoá đợt sử dụng thuốc BVTV này`,
            '',
            [
                {
                    text: 'Huỷ',
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <NormalFlashList
                ...
                emptyViewProps={{
                    bigTitle: 'Danh sách thuốc BVTV rỗng',
                    bigTitleStyle: {
                        color: 'black',
                        fontSize: 14
                    }
